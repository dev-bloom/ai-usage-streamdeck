...
- **`SDKVersion` is `2`; the schema's description for this field reads
  "Preferred SDK version (**version 3 is recommended**)."** The schema still
  accepts `2` (`enum: [2, 3]`), so this isn't a hard failure, but it's a
  documented recommendation the manifest currently doesn't follow. Worth a
  deliberate decision, not silence — see open questions.
...
|---|---|---|---|
| `imgs/plugin/marketplace.png` (top-level `Icon`) | PNG, 256×256 | **288×288** | **Mismatch — needs regenerating at 256×256** |
| `imgs/plugin/marketplace@2x.png` | PNG, 512×512 (@2x) | **576×576** | **Mismatch — needs regenerating at 512×512** |
...
7. **`SDKVersion 2` vs the schema's "version 3 is recommended."** Confirm
   whether to migrate before submission or submit as-is; either is valid per
   the schema, but it should be a decision, not an oversight.
8. **Plugin icon dimensions.** `imgs/plugin/marketplace.png` and
   `marketplace@2x.png` are 288×288 / 576×576, not the schema-declared
   256×256 / 512×512 — see the icon table above. Needs regenerating before
   submission; not something this document can fix on its own.
